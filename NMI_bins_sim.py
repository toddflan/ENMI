# Localization testing with simulated images
...
def test_location(size, bins, variance, cols_per_row):
    ...
    return ((true_value_NMI - candid_value_NMI) / true_value_NMI, (true_value_ENMI - candid_value_ENMI) / true_value_ENMI) # get Exp[true_NMI - candid] val as a % of true value
...
if __name__ == "__main__":
    ...
    for i in range(bin_array.size):
        bin_array[i] = 2**(i + 1)
    bin_array = bin_array.astype(np.int)
    
    # arrays to store decision gap
    gaps_NMI = np.zeros(bin_array.size)
    gaps_ENMI = np.zeros(bin_array.size)
    
    start_time = time.time()

    for iter in range(gaps_NMI.size):
        old_time = time.time()
        
        print((iter + 1), "/", gaps_NMI.size, end=' ', flush=True)
        
        var_wo_z = (2 * N_0 * np.cos(camera_angle)) / (focal_length**2 * camera_height * side_length)
        var_array = var_wo_z * z_bounds
        
        # run a bunch of times
        num_of_samples = 1000
        
        # list for storing pool results
        result_list = []
        ...
        for i in range(num_of_samples):
            ...
            result_list.append(pool.apply_async(func=test_location, args=(num_squares, bin_array[iter], var_array, sample_cols_per_row, )))
        ...
        for i in range(num_of_samples):
            ...
            # print(result)
            sum_gaps_NMI += result[0]
            sum_gaps_ENMI += result[1]
        
        # get gaps average
        gaps_NMI[iter] = sum_gaps_NMI / num_of_samples
        gaps_ENMI[iter] = sum_gaps_ENMI / num_of_samples
    ...
    # save to csv
    # df.to_csv('NMI_results_bins_005.csv', index=False)
    
    # plot
    plt.plot(bin_array, gaps_NMI, bin_array, gaps_ENMI)
    plt.legend(['NMI Gap', 'ENMI Gap'])
    plt.title('Exp. Decision Gap (%)')
    plt.show()